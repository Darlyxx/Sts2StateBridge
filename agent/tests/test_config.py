...
def test_settings_load_from_file(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.delenv("STS2_SKILL_PATH", raising=False)
    env = tmp_path / ".env"
    ...
    assert settings.skill_path.name == "sts2-ironclad-player"
...
def test_settings_accept_skill_path_override(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("LLM_API_KEY", raising=False)
    directory = tmp_path / "custom-skill"
    env = tmp_path / ".env"
    env.write_text(
        f"LLM_API_KEY=test-key\nSTS2_SKILL_PATH={directory}\n",
        encoding="utf-8",
    )
    settings = Settings.from_env(env)
    assert settings.skill_path == directory.resolve()
