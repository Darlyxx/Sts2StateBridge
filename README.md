...
当前接受快照候选中的战斗动作，以及奖励领取、卡牌奖励、药水丢弃、宝箱、休息点、锻造、地图移动、事件选择和商店动作。`0.13.0` 还支持战斗临时选牌、卡牌组合、手牌/牌堆多选、确认、取消、跳过和卡牌附魔选择。选择弹窗存在时只返回 `selection:*` 候选，不会混入普通战斗动作。非战斗候选位于 `interaction.actions`。请求必须同时携带最新的 `state_id` 与 `action_id`；同一个状态只接受一次动作。真实本机配置不会提交到 Git。

卡牌结果展示页（`NSimpleCardsViewScreen`）会提供 `proceed` 动作，对应界面上的“√”确认按钮。商店中名为“空值”的条目是游戏卡牌 `Null`，不是缺失数据。

统一选择动作的 ID 为 `selection:select:<instance_id>`、`selection:deselect:<instance_id>`、`selection:confirm`、`selection:cancel` 和 `selection:skip`。单选自动确认界面只执行选择；需要手动确认的多选或附魔界面会在下一份快照中单独提供确认动作。
